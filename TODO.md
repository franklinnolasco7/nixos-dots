...
// ⚠ TEMPORARY GATE (log — do not forget): disko-module mounts are gated to the
// vm host only; hosts/aspire7/hardware-configuration.nix carries the live-disk
// UUIDs again so `nixos-rebuild switch --flake .#aspire7` is safe on the live
// laptop while iterating. Disk-layout changes are still rehearsed in the VM
// (useDiskoMounts = true); non-disk changes test via a real switch.
// REVERT when installing for real (install.sh / nixos-anywhere):
//   1. flake.nix: re-enable `useDiskoMounts = true` for aspire7 (drop the gate
//      comment) — mounts come from disko.nix again at build time
//   2. hosts/aspire7/hardware-configuration.nix: restore the UUID-free version
//      (regenerated by nixos-anywhere during install)
//   3. hosts/aspire7/disko.nix: remove the TEMPORARY note
